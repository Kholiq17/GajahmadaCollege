...
$id = $_GET['id'];
$ambil = mysqli_query($koneksi, "SELECT * FROM `postingan` WHERE `id` = '$id'");
			...
			<div class="col-md-12">
				<div class="card card-user">
					<div class="card-header">
						<h5 class="card-title">Update Post : <?= $judul ?></h5>
					</div>
					<div class="card-body">
						<form action="proses/editpostingan.php?id=<?= $id ?>" method="post">
							<div class="form-group">
								<label>Judul</label>
								<input type="text" class="form-control" placeholder="Judul Postingan" value="<?= $judul ?>" name="judul">
							</div>
							<div class="form-group">
								<label>Postingan</label>
								<textarea name="content" id="editor" placeholder="Isi Postingan disini"><?= $isi ?></textarea>
							</div>
